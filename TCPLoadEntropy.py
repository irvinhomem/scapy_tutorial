...
TCPpktbytes = [bytes(pkt[IP][TCP][Raw].load) for pkt in pktcap if TCP in pkt and Raw in pkt[TCP]]
TCPpktbytesFreq = [Counter(bytes(pkt[IP][TCP][Raw].load)) for pkt in pktcap if TCP in pkt and Raw in pkt[TCP]]
...
#perPktCharEntropySeq = [CalcEntropy(Counter(bytes(pkt[IP][TCP][Raw]))) for pkt in pktcap if TCP in pkt and Raw in pkt and pkt[TCP].dport==21]
perPktCharEntropySeq = [CalcEntropy(Counter(bytes(pkt[IP][TCP][Raw].load))) for pkt in pktcap if TCP in pkt and Raw in pkt]
